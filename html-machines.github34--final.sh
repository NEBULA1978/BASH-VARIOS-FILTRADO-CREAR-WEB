...
function getskill(){
  ...
  echo -e "\n${redColour}[!]Se va a buscar por la slill $skill${endColour}\n"
  check_skill="$(cat bundle.js | grep "skills: " -B 6 | grep "SNMP Enumeration" -i -B 6 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column)"
  # Si tienes contenido
  if [ "$check_skill" ]; then
    echo -e "\n${greenColour}[!]Se representan las maquinas donde se toca la $skill  {endColour}\n"
...
while getopts "m:ui:y:d:o:s:h" arg; do
  ...
  o)
    ...
    ;;
  s)
    skill="$OPTARG";let parameter_counter+=7
    ;;
  h) ;;
  esac
done
