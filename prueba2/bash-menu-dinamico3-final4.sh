...
while true; do
    ...
    echo "6. Salir"
    echo "7. Buscar texto en el archivo de búsqueda."
    read -p "Seleccione una opción (1/2/3/4/5/6/7): " choice
    ...
    7)
        ...
        read -p "Introduzca el texto a buscar: " text_to_search

        # Perform the search using grep and display matching lines
        grep -n "$text_to_search" "$buscar_file" # Use -n to display line numbers
        ;;
    6)
        ...
        ;;
    *)
        echo "Opción no válida. Por favor, seleccione una opción válida (1/2/3/4/5/6/7)."
        ;;
    esac
done
