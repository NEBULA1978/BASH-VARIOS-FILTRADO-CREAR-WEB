    ...
    8)
        ...
        if [ "$option_insert" = "s" ]; then
            ...
            if [ -f "$insert_file" ]; then
                awk -v line="$insert_line" -v file="$output_file" 'NR == line {while ((getline var < file) > 0) print var} {print}' "$insert_file" >temp_file && mv temp_file "$insert_file.temp"
                mv "$insert_file.temp" "$insert_file"
                echo "Resultados insertados exitosamente en la línea $insert_line de '$insert_file'."
            else
                echo "El archivo '$insert_file' no existe. No se pueden insertar los resultados."
            fi
        fi
        ;;
